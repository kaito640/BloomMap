...
def preprocess_word(word):
    ...
    # Skip common biblical numbers that aren't meaningful
    if cleaned in {'one', 'two', 'three', 'four', 'five', 'six', 'seven'}:
        return None
...
def get_cluster_themes(cluster_docs, vectorizer, nmf_model, n_themes):
    ...
    Parameters:
    -----------
    cluster_docs : array-like
        Document vectors for the cluster
    vectorizer : TfidfVectorizer
        The fitted vectorizer containing vocabulary
    nmf_model : NMF
    ...
    --------
    list of tuples
        (word, importance_score) pairs
    """
    # Get feature names (words) from vectorizer
    feature_names = vectorizer.get_feature_names_out()
    
    # Average the topic distributions for this cluster
    cluster_topic_dist = np.mean(cluster_docs, axis=0)
    
    # Collect themes with importance scores
    themes = []
    for topic_idx in range(len(cluster_topic_dist)):
        # Get the components (word weights) for this topic
        topic_weights = nmf_model.components_[topic_idx]
        
        # Get top words for this topic based on weights
        top_word_indices = topic_weights.argsort()[-10:][::-1]  # Get top 10 words
    ...
    # Remove duplicates, keeping highest importance score
    theme_dict = {}
    for word, score in themes:
        if word not in theme_dict or score > theme_dict[word]:
            theme_dict[word] = score
    
    # Sort by importance and take top N
    final_themes = sorted(theme_dict.items(), key=lambda x: x[1], reverse=True)
    return final_themes[:n_themes]
        ...
        if need_download:
            ...
            try:
                ...
                metadata.update({
                    ...
                    'file_hash': self.get_file_hash(self.bible_path),
                    'source_url': self.source_url
                })
...
class GPUAcceleratedPipeline:
    ...
    def process_sections(self, sections, progress_callback=None):
        """Process all sections with progress tracking"""
        ...
        # Create vectorizer for feature names
        from sklearn.feature_extraction.text import CountVectorizer
        vectorizer = CountVectorizer(max_features=1000)
        vectorizer.fit(texts)
        feature_names = vectorizer.get_feature_names_out()
        
        # Use sklearn's NMF with progress updates
        from sklearn.decomposition import NMF
        print("\nPerforming NMF...")
        nmf = NMF(n_components=N_TOPICS, random_state=42, max_iter=200)
        nmf_output = nmf.fit_transform(np.abs(embeddings))
        
        return nmf_output, feature_names


    def estimate_memory(self, n_sections):
        """Estimate memory requirements"""
        # Rough estimation of memory needs
        embedding_size = 768  # BERT embedding size
        bytes_per_float = 4
        estimated_memory = n_sections * embedding_size * bytes_per_float
        return estimated_memory / (1024 * 1024 * 1024)  # Convert to GB
...
def get_cluster_themes(cluster_docs, topic_words, n_themes):
    themes = []
...
class ProgressLogger:
    ...
    def update(self, step_name, step_count=1):
        ...
        if len(self.step_times) > 10:  # Keep last 10 for moving average
        ...
        if self.total_steps:
            ...
            estimated_remaining = steps_remaining * avg_step_time
            completion_time = datetime.now() + timedelta(seconds=estimated_remaining)
            
            print(f"\n{step_name}:")
            print(f"Progress: {self.current_step}/{self.total_steps} ({(self.current_step/self.total_steps*100):.1f}%)")
...
def main():
    ...
    with tqdm(total=n_sections, desc="Generating embeddings") as pbar:
        nmf_output = gpu_pipeline.process_sections(sections, progress_callback=pbar.update)
    print(f"Embeddings generation complete. Time taken: {time.time() - embedding_start:.2f}s")
    
    # Step 3: Perform NMF
    print("\nStep 3/5: Performing NMF...")
    nmf_start = time.time()
    nmf_model = NMF(n_components=N_TOPICS, random_state=42, max_iter=300)  # Increased iterations
    nmf_output = nmf_model.fit_transform(tfidf_matrix)
    print(f"NMF complete. Time taken: {time.time() - nmf_start:.2f}s")
    
    # Step 4: Find optimal clusters
    print("\nStep 4/5: Finding optimal number of clusters")
    cluster_start = time.time()
    n_clusters = find_optimal_clusters(nmf_output, MAX_CLUSTERS)
    print(f"Optimal number of clusters: {n_clusters}")
    print(f"Cluster optimization complete. Time taken: {time.time() - cluster_start:.2f}s")
    ...
    labels = clusterer.fit_predict(nmf_output)
