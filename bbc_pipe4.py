...
def process_data(file_path, n_clusters=10, m_subclusters=3, top_n=50, bottom_x=100):
    ...
    for i in range(n_clusters):
        ...
        if actual_subclusters > 1:
            ...
            sub_labels = subkmeans.fit_predict(cluster_docs)
            df.loc[df['cluster'] == i, 'subcluster'] = sub_labels
            subcluster_words.append(get_top_words(vectorizer, subkmeans, top_n))
        else:
            # If we can't subcluster, just use the main cluster's words
            df.loc[df['cluster'] == i, 'subcluster'] = 0
            subcluster_words.append([top_cluster_words[i]])
    
    return df, word_freq, top_cluster_words, subcluster_words, most_common_words

def save_outputs(df, word_freq, top_cluster_words, subcluster_words, most_common_words, output_dir):
    os.makedirs(output_dir, exist_ok=True)
    
    # Save document to cluster map
    df[['document_id', 'cluster', 'subcluster']].to_csv(f"{output_dir}/document_to_cluster_map.csv", index=False)
...
if __name__ == "__main__":
    ...
    df, word_freq, top_cluster_words, subcluster_words, most_common_words = process_data(file_path)
    save_outputs(df, word_freq, top_cluster_words, subcluster_words, most_common_words, output_dir)
